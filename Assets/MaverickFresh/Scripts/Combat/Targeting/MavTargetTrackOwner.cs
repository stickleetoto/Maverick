...
namespace MaverickFresh.Combat.Targeting
{
    ...
    ///
    /// WHY THIS EXISTS. Before it, the target set was whatever `FindObjectsOfType` returned at the
    /// moment each consumer happened to ask. The scene graph WAS the track database, so nothing
    ...
    /// got ground truth rather than sensor output, which meant no sensor limitation could ever be
    /// modelled.
    ///
    /// Deliberately NOT here, because they are later phases: detection, scan volumes, lock
    /// semantics, sensor fusion between feeds, identification, seekers, guidance.
    /// </summary>
    ...
    public sealed class MavTargetTrackOwner : MonoBehaviour
    {
        ...
        private readonly List<IMavTargetObservationFeed> feeds = new List<IMavTargetObservationFeed>(4);
        ...
        /// <summary>
        /// Registers a feed. Idempotent, so a caller that cannot easily tell whether it already
        /// registered does not create a duplicate - which would double every observation.
        /// </summary>
        ...
        /// <summary>
        /// Finds the track a feed's key maps to, so a legacy authority holding a concrete object can
        /// say which track it means without the owner exposing its correlation table.
        /// </summary>
        public bool TryResolveTrackId(MavTrackSource source, int sourceKey, out int trackId)
        {
            trackId = 0;
            if (sourceKey == 0)
                return false;
            ...
            int index;
            if (!correlation.TryGetValue(MakeKey(source, sourceKey), out index))
                return false;
            if (index < 0 || index >= tracks.Count)
                return false;
        ...
        private void Sweep()
        {
            ...
            {
                IMavTargetObservationFeed feed = feeds[i];
                if (feed == null || !feed.IsFeedActive)
                    continue;
                feed.CollectObservations(observationScratch);
            }

            debugObservationsLastSweep = observationScratch.Count;
            float now = Time.time;

            for (int i = 0; i < observationScratch.Count; i++)
                Integrate(observationScratch[i], now);
        }

        private void Integrate(MavTrackObservation observation, float now)
        {
            if (observation.sourceKey == 0)
                return;

            long key = MakeKey(observation.source, observation.sourceKey);
            int index;
            ...
            if (!correlation.TryGetValue(key, out index))
            {
                ...
                created.data.source = observation.source;
                ApplySample(ref created, observation, now, false);
                tracks.Add(created);
            ...
            ApplySample(ref entry, observation, now, true);
            tracks[index] = entry;
        }
        ...
        /// </summary>
        private void ApplySample(ref MavTrackEntry entry, MavTrackObservation observation, float now, bool hadTrack)
        {
            entry.displayName = observation.displayName;
            entry.team = observation.team;
            entry.isAirTarget = observation.isAirTarget;
            entry.data.source = observation.source;
        ...
        private static long MakeKey(MavTrackSource source, int sourceKey)
        {
            return ((long)source << 32) ^ (uint)sourceKey;
        }
