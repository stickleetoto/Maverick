...
**Checkpoint:** F-15 V1 freeze. This is a coherent frozen baseline. Follow-on opportunities are ranked in `F15_POST_FREEZE_OPPORTUNITIES_V1.0.md`.
**Research baseline:** **F-15 AFIT/BAUMANN/DAVISON RESEARCH BASELINE V1 — FROZEN** (`F15_RESEARCH_BASELINE_FREEZE_V1.0.md`). The research configuration was flown through the Unity runtime path in Play Mode, held its source equilibria, and matched the source RK4 up to first-order integration error. No further research work package is required; what remains is optional (D16, D3) or a new phase (pilot-controlled F-15).
...
| **Research runtime flight prerequisites (WP-4A)** | **built and verified statically, NOT flown** | research | `F15_RESEARCH_RUNTIME_PREREQUISITES_V1.0.md` | **research** | **Environment (opt-in, OFF by default):** the research profile can supply the source's fixed density RHO 0.6531396 kg/m³ (D11) and source G 9.8066352 m/s² through the load set, once, with `useGravity` off. `MavAtmosphereModel` and `Physics.gravity` are untouched; a refused override makes the body apply nothing. **Surfaces:** a source-defined static stabilator is held by the F-15 actuator (still the single owner), with no travel, rate or gearing. **Initialization:** the body's new `TryApplyInitialKinematicState`, before arming only. **Contamination:** one authority grants exactly the research id; NASA 836 is refused by name. **Verified:** all 170 equilibria inject and read back to float precision; the body's own loads balance there to 3.9e-6 g in the source environment (8.0e-7 g after the documented mass convention), vs 3.4e-4 g with density only and 1.75e-3 g with neither. So equivalence needs density + gravity. |
| **Research runtime closeout — Unity Play Mode (final)** | **FLOWN AND VALIDATED — BASELINE FROZEN** | research | `F15_RESEARCH_BASELINE_FREEZE_V1.0.md` | **research** | **Ownership:** new owner `F15AfitResearch` (validation only, safety hold OFF by default). It admits exactly the research id with source density AND source gravity and one armed body, and faults the step either stops holding. **Play Mode (deterministic, captureFramerate 50, byte-identical reruns), 39 / 0:** symmetric point 36 held to 1.7e-7 for 30 s. Turning point 150 held to 1.9e-4 / 9.5e-5 / 4.8e-5 at dt / dt/2 / dt/4, with ψ̇ −0.030652 → −0.030649 vs WP-3C −0.030648. Unity − source RK4 is first order in dt (orders 0.95–1.08). Frame/sign/Ixz: (ṗ, q̇, ṙ) equals the source RHS to 3e-7, where a flipped Ixz would be 1.7e-2 away; the gyro-off control departs 13×. Environment loss → Fault, no load. **Runtime bugs found: none** (harness-only fixes). |
| Live-flight readiness | exact: **FAIL-CLOSED** · research: **validation owner only (Play Mode, temporary rigs)**, not gameplay | — | — | — | exact: profile invalid → not structurally prepared → no live or shadow loads. Research (WP-1): the separate profile `F15_AFIT_BAUMANN_DAVISON_MACH06_20K_RESEARCH` reaches STRUCTURALLY_PREPARED at M 0.6 / 20,000 ft only, with its propulsion non-authoritative and not accepted. F15Replacement ownership is not implemented; no scene or prefab is wired. **Closeout:** the research body can be the sole live owner through `F15AfitResearch` for validation only. |
...
| research runtime overrides never reach a non-research body | `MavF15ResearchRuntimeAuthority` grants exactly the research id (ordinal) on a body that flies the research profile; the environment, static hold and injection all consult it; a refused environment makes the body apply nothing, and a refused hold holds neutral | `[U3]`, `[U13]` |
| the default environment and the F-16 are unchanged | the base provider returns the shared atmosphere sample as given; the gravity channel and `AppliedForceAeroBodyN` are inert without an override; the 22 synchronous F-16/shared baseline suites pass 1,247 / 0 before and after, reports line-identical but for a scan count and a telemetry-noise reading | `[U1]`, `[U2]` |
