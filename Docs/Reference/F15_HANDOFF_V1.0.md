...
known_good:        401 passed / 0 failed (5 headless suites); 319 scripts compile, 0 errors
playmode_flight:   NOT RUN
exact_path:        FAIL-CLOSED
research_path:     aero + normalized thrust shape only; not flyable (no research profile, zero thrust)
```
...
`MavF15MassReferenceValidation` (45) · `MavF15ReferenceGeometryValidation` (51) · `MavF15BaumannTranscriptionValidation` (28) · `MavF15ControlPathValidation` (79) · `MavF15PropulsionValidation` (198)
...
- Research-model constants (e.g. Baumann's 8,300 lb trim thrust) inside R5 propulsion.
...
2. Run the five suites (validation plan §1) and confirm **401 / 0**.
3. Pick up work package **WP-1** from `F15_POST_FREEZE_OPPORTUNITIES_V1.0.md` §5.
