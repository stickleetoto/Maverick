...
| D17 | *(new, WP-3D)* **Staged research-only flight.** (1) Integrate the source RHS off the body, with no Rigidbody, and check the eigenvalue predictions in time: holds at stable points, ≈ 46 s doubling at the saddles, ≈ 0.85 s doubling at the α 13° pair. (2) Only then fly a Rigidbody, after D11, research surface travel and fixed-thrust semantics on the body. Either result compares against the SOURCE MODEL, never the aircraft. | **HIGH VALUE** (research) after D11 | M |
...
- **Next candidates:** the CMMQ question (D3); pseudo-arclength continuation (D16); a staged research flight (D17, after D11).
