...
| D11 | ✅ **DONE (WP-4A)** — *(new, WP-3A; design in WP-3B)* **Research-only runtime environment policy** for `SourceReproduction`: fixed source density, and a gravity decision. **WP-4A:** opt-in `SourceFixedDensity` (RHO 0.6531396 kg/m³ — the WP-3B doc's "0.653142" was a typo) and `SourceGravity` (through the load set, `useGravity` off). Research-profile-scoped, OFF by default, fail-closed; `MavAtmosphereModel` and `Physics.gravity` untouched. Equivalence needs both: at the 170 injected equilibria the body balances to 3.9e-6 g with both, 3.4e-4 g with density only (`F15_RESEARCH_RUNTIME_PREREQUISITES_V1.0.md`). | **HIGH VALUE** before any flying source reproduction | The source has no altitude state: its density is the 20,000-ft constant at every state. `MavSixDoFBody` samples ISA density at its altitude, so runtime `SourceReproduction` does not reproduce the source's q (6.83e-4 low even at 6,096 m; measured in `[R13]`). Unity's gravity is 9.81 against the source's 9.80664 m/s² (+3.43e-4). WP-3B solved trim off the body in the source's own environment. The policy — research-profile-scoped, default off, never touching `MavAtmosphereModel` — is designed in `F15_RESEARCH_TRIM_SOLVER_V1.0.md` §10 and **not built**. Gravity touches the load-application boundary and needs its own reviewed change. | S–M |
...
- **WP-3C — ✅ COMPLETE** (turning / helical trim with φ fixed, off the body). `F15_RESEARCH_TURNING_TRIM_V1.0.md`.
  - 80 non-symmetric states: 320 / 320 converged, all within print floor + numerical uncertainty.
  - Pitchfork point 165 found from the data and solved symmetric.
  - Mirror parity exact; no second root found; CFX2 inactive at every root.
- **WP-3D — ✅ COMPLETE** (source stability / eigenvalue reproduction, off the body). `F15_RESEARCH_STABILITY_ANALYSIS_V1.0.md`.
  - The source's FUNX is reproduced exactly and linearized at all 170 equilibria: 127 stable, 40 unstable, 3 near-neutral.
  - **Disagreements with Table VII's caption:** the fold-bounded saddles (structural) and the printed-CMMQ longitudinal pair at α 13.0–14.1°.
  - **Reproduced as eigenvalue zeros:** the printed folds and the pitchfork (supercritical). Baumann's −19.73° Hopf is found at −19.33°.
  - Mirror spectra identical; dataset in `Data/F15/stability/`.
- **WP-3E — ✅ COMPLETE** (nonlinear time-domain verification + source stability conflict audit). `F15_RESEARCH_TIME_DOMAIN_STABILITY_V1.0.md`, `F15_RESEARCH_STABILITY_CONFLICT_AUDIT_V1.0.md`.
  - **Agreement:** RK4 on the unchanged source RHS reproduces every tested WP-3D eigenvalue within 1 %.
  - **Confirmed nonlinearly:** the CMMQ band grows, the fold saddles are real, the pitchfork breaks symmetry, and mirrored trajectories are bit-identical.
  - **Conflict verdict:** PUBLIC PRINTINGS AGREE — EXECUTED AUTO MODEL MAY DIFFER. No model change.
  - **The source-model research path is frozen as a research reference:** as printed, and characterized statically, in trim, in eigenvalues and in time.
- **WP-4A — ✅ COMPLETE** (research runtime flight prerequisites, NOT flown). `F15_RESEARCH_RUNTIME_PREREQUISITES_V1.0.md`.
  - **Environment:** D11 source fixed density and source gravity through the load set, opt-in and fail-closed. The default path and the F-16 are bit-identical.
